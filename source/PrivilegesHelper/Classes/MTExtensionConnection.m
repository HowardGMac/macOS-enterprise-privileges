...
@interface MTExtensionConnection ()
...
@implementation MTExtensionConnection

- (void)connectToExtensionAndExecuteCommandBlock:(void(^)(void))commandBlock
{
    if (!_connection) {
        
        _connection = [[NSXPCConnection alloc] initWithMachServiceName:kMTExtensionMachServiceName
                                                               options:NSXPCConnectionPrivileged
        ];
        [_connection setRemoteObjectInterface:[NSXPCInterface interfaceWithProtocol:@protocol(PrivilegesExtensionProtocol)]];
...
#pragma clang diagnostic ignored "-Warc-retain-cycles"
        [_connection setInvalidationHandler:^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                os_log(OS_LOG_DEFAULT, "SAPCorp: Extension connection invalidated");
        ...
        [_connection setInterruptionHandler:^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                os_log(OS_LOG_DEFAULT, "SAPCorp: Extension connection interrupted");
                self->_connection = nil;
            });
        }];
